  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    iconPositions.forEach((icon) => {
      ...
      if (dx * dx + dy * dy < radius * radius) {
        ...
        setTargetRotation({
          x: targetX, y: targetY, startX: currentX, startY: currentY,
          distance, startTime: performance.now(), duration: Math.min(2000, Math.max(800, distance * 1000)),
        })
      }
    })
    setIsDragging(true)
